...
FOR options_row IN (select * from json_populate_recordset(null::groups, options)) LOOP
	...
	-- criteria: code
	IF options_row.code IS NOT NULL THEN
		IF multi_criteria = false THEN  command = command || ' WHERE'; multi_criteria := true;
		ELSE command = command || ' AND';
		END IF;

		command = format(command || ' code = %L', options_row.code);
	END IF;

	-- criteria: name
	IF options_row.name IS NOT NULL THEN
		IF multi_criteria = false THEN  command = command || ' WHERE'; multi_criteria := true;
		ELSE command = command || ' AND';
		END IF;

		options_row.name := '%' || options_row.name || '%';
		command = format(command || ' name LIKE %L', options_row.name);
	END IF;
